...
class UtNet2(Denoiser):
    def __init__(
        ...
        self.dec3  = nn.Conv2d( funit+4,   funit, 3, padding=pd, padding_mode=pm)
    ...
    def forward(self, I):
        x  = self.relu(self.con0a(I))
        l1 = self.pool(self.relu(self.enc0(x)))
        x  = self.relu(self.con1a(l1))
        l2 = self.pool(self.relu(self.enc1(x)))
        x  = self.relu(self.con2a(l2))
        l3 = self.pool(self.relu(self.enc2(x)))
        x  = self.relu(self.con3a(l3))
        ...
        x  = self.relu(self.dec3(torch.cat((x, I), 1)))
